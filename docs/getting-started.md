...
- **Аккаунт Gmail**: если хотите, чтобы агент Почтальон обрабатывал вашу почту Gmail (через GWS CLI или MCP)
- **Аккаунт Hey.com**: если вы используете Hey для почты (через Hey CLI) — работает вместе с Gmail или вместо него
...
2. **Используете ли вы Gmail, Hey.com или Google Calendar?** Выберите «да» для настройки интеграции с Почтальоном
...
- Хотите разбор почты? (требуется Gmail через GWS/MCP или Hey.com через Hey CLI)
- Хотите интеграцию с календарём? (требуется Google Calendar через GWS/MCP)
...
Навык `/email-triage` просканирует ваш почтовый ящик (Gmail или Hey.com), сохранит письма, требующие действий, и выдаст сводку.
...
### «Почта/Календарь не работает»
Почтальону нужен хотя бы один бэкенд почты: GWS CLI (`gws`), Hey CLI (`hey`) или MCP-коннекторы. Для GWS смотрите `docs/gws-setup-guide.md`. Для Hey установите с [github.com/basecamp/hey-cli](https://github.com/basecamp/hey-cli) и выполните `hey auth login`. Для MCP запустите установщик повторно (`bash scripts/launchme.sh`) и ответьте **да** на вопрос о Gmail/Calendar, или вручную скопируйте `.mcp.json` из репозитория в корень хранилища.
